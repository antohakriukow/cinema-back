...
export class GenreService {
	...
	async getCollections() {
		...
		const collections = await Promise.all(
			genres.map(async (genre) => {
				const moviesByGenres = await this.movieService.byGenres([
					genre._id,
				])

				const bigPoster =
					moviesByGenres.length > 0 ? moviesByGenres[0].bigPoster : ''
	...
	async byId(_id: string) {
		...
		if (!genre) throw new NotFoundException('Model not found')
